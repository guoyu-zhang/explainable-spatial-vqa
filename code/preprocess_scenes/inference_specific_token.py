...
SUBSET_FRACTION = 0.1  # Use only 10% of the data; adjust as needed
        ...
        with h5py.File(annotated_h5_path, 'r') as hf:
            for key in self.question_keys:
                ...
                for step_idx, step in enumerate(annotated_program):
                    ...
                    # Only include token samples (i.e. output does NOT contain '[')
                    if '[' in raw_out:
                        continue
...
def run_inference_for_function_2():
    # Select device.
    ...
    # Print 10 inference examples for function number 2.
    examples_printed = 0
    with torch.no_grad():
        for image_feat, func_token, input_bboxes, target_token in dataloader:
            # Filter: only consider samples where function token equals 2.
            # Note: func_token is a tensor of shape (batch_size,); we check each item.
            for i in range(image_feat.size(0)):
                if func_token[i].item() != 2:
                    continue
                # Move individual sample to device.
                img = image_feat[i].unsqueeze(0).to(device)
                func = func_token[i].unsqueeze(0).to(device)
                boxes = input_bboxes[i].unsqueeze(0).to(device)
                tgt = target_token[i].item()

                logits = model(img, func, boxes)
                pred_token = logits.argmax(dim=1).item()
                
                logging.info(f"Inference Example {examples_printed+1}:")
                logging.info(f"Function: {func_token[i].item()}")
                logging.info(f"Predicted token: {pred_token}")
                logging.info(f"Target token: {tgt}")
