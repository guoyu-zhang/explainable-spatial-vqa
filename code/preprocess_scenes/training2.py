...
class CLEVRDataset(Dataset):
    def __init__(self, questions_file, features_file, vocab_file, max_seq_len=20, subset_frac=1.0):
        ...
        # Dynamically determine bounding box functions.
        # Here we consider functions that return an ObjectSet to be those whose names start with
        # "scene", "relate[", "filter_", "same_" or are exactly "unique".
        self.bbox_functions = set()
        for fn in self.vocab.keys():
            if fn.startswith("scene") or fn.startswith("relate[") or fn.startswith("filter_") or fn.startswith("same_") or fn == "unique":
                self.bbox_functions.add(fn)

        # Other target types.
        self.integer_functions = set(["count"])
        self.boolean_functions = set(["exist", "equal_integer", "less_than", "greater_than"])
        self.size_functions = set(["query_size"])
        self.color_functions = set(["query_color"])
        self.shape_functions = set(["query_shape"])
        self.material_functions = set(["query_material"])
        ...
        with h5py.File(self.questions_file, "r") as f:
            ...
            for key in keys:
                ...
                for step in prog:
                    ...
                    if function_str in self.bbox_functions:
                        target_type = "bbox"
                    elif function_str in self.integer_functions:
                        target_type = "integer"
                    elif function_str in self.boolean_functions:
                        target_type = "boolean"
                    elif function_str in self.size_functions:
                        target_type = "size"
                    elif function_str in self.color_functions:
                        target_type = "color"
                    elif function_str in self.shape_functions:
                        target_type = "shape"
                    elif function_str in self.material_functions:
...
def main():
    ...
    num_debug_samples = 3  # Change this number if you want more or fewer samples.
...
# ##############################
# # Main Training and Inference Routine
# ##############################
...
#     # Run inference on a few samples.
#     # For demonstration, we'll pick the first 3 samples from the dataset.
#     print("\nInference Examples:")
#     for i in range(min(3, len(dataset))):
#         sample = dataset[i]
#         run_inference(model, sample, device, vocab)
