...
# --- Modified Training Loop with Early Stopping and Loss Logging ---
def train():
    ...
    # Open a file to log training losses.
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # Check if current epoch improved the loss
        if avg_epoch_loss < best_loss:
            ...
            logging.info(f"No improvement in epoch {epoch+1}. Patience counter: {epochs_since_improvement}/{patience}")
    ...
    with torch.no_grad():
        for i, (img_feat, func_token, in_bboxes, target_bboxes, bbox_mask, raw_output) in enumerate(inference_loader):
            ...
            if branch_pred.item() != 1:
                continue  # Skip non-token examples
    ...
    with torch.no_grad():
        for i, (img_feat, func_token, in_bboxes, target_bboxes, bbox_mask, raw_output) in enumerate(inference_loader):
            ...
            if branch_pred.item() != 0:
                continue  # Skip non-bbox examples
            # For bbox samples, show the predicted bounding boxes and target bounding boxes.
            predicted_boxes = bbox_out[0].cpu().numpy()  # shape: (MAX_OUTPUT_BOXES, 5)
            target_boxes = target_bboxes[0].cpu().numpy()  # shape: (MAX_OUTPUT_BOXES, 4)
