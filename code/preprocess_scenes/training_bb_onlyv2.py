...
IOU_LOSS_WEIGHT = 1.0  # Weight for IoU loss
...
def compute_iou_loss(pred_boxes, target_boxes, mask, eps=1e-6):
    """
    Computes the IoU loss for each predicted box compared to target box.
    pred_boxes, target_boxes: (B, N, 4) tensors in [xmin, ymin, xmax, ymax] format.
    mask: (B, N) tensor with 1 for valid boxes and 0 for padded boxes.
    Returns: average IoU loss over valid boxes.
    """
    # Unbind coordinates
    ...
    iou_loss = (1 - iou) * mask  # Only consider valid boxes
...
def train():
    ...
    # For coordinate regression: original MSE loss (per coordinate)
    ...
    for epoch in range(NUM_EPOCHS):
        for i, (img_feat, func_token, in_bboxes, target_bboxes, bbox_mask) in enumerate(dataloader):
            ...
            # MSE coordinate loss: computed per coordinate
            ...
            # IoU loss: computed only on valid boxes
            loss_iou = compute_iou_loss(pred_coords, target_bboxes, bbox_mask)
            
            # Combine coordinate losses (you may adjust the weighting)
            loss_coord_total = loss_coords + IOU_LOSS_WEIGHT * loss_iou
            ...
            total_loss = loss_coord_total + CONF_LOSS_WEIGHT * loss_conf
            total_loss.backward()
            optimizer.step()
            
        logging.info(f"Epoch [{epoch+1}/{NUM_EPOCHS}], Step [{i+1}/{len(dataloader)}], Loss: {total_loss.item():.4f}")
        
        # Save a checkpoint at the end of each epoch if the interval is met.
